...
from kyberos.core.system_logger import SystemLogger

logger = logging.getLogger("kyberos.brain.rlm")
...
class RLMEngine:
    ...
    def __init__(
        ...
        protocol_manager: Optional[Any] = None, # Avoid circular import if needed
    ...
    async def think(self, user_query: str, depth: int = 0, session_id: Optional[str] = None, model_tier: str = "smart_model", source: str = "USER") -> str:
        ...
        2. Gathers context (Thread-Shift).
        ...
        while current_turn < max_turns:
            ...
            # (e.g. new skills created, THREAD.md edits).
            ...
            for tool_call in tool_calls:
                ...
                if fn_name == "spawn_sub_agent":
                    ...
                    try:
                        ...
                        elif self.tool_registry and fn_name in self.tool_registry._skills:
                             # Check if it's a spell
                             result = await self.tool_registry.execute_tool(fn_name, args)
                             result_content = f"Spell {fn_name} executed: {result}"
                        elif self.protocol_manager and fn_name in self.protocol_manager.get_tool_names():
                            result = await self.protocol_manager.execute_tool(fn_name, args)
    ...
    def _assemble_system_prompt(self, task_context: TaskContext, live_categories: Optional[set[str]] = None) -> str:
        ...
        agent_text = self._read_section(KYBEROS_ROOT / "AGENT.md")
        parts.append(agent_text if agent_text else "You are Kyberos, a recursive AI agent.")

        # 1. The Soul (Personality)
        if soul_text := self._read_section(KYBEROS_ROOT / "SOUL.md"):
            parts.append(soul_text)

        # 2. The User (Only at Depth 0)
        if task_context.depth == 0:
            if user_text := self._read_section(KYBEROS_ROOT / "USER.md"):
        ...
        if task_context.relevant_snippets:
            parts.append("## Relevant Context (Archive)")
    ...
    def _get_available_tool_names(self) -> list:
        """Collects all known tool names from registry, skills, protocols, and builtins."""
        names = set()
        names.add("spawn_sub_agent")
        if self.tool_registry:
            names.update(self.tool_registry._internal_tools.keys())
            names.update(self.tool_registry._skills.keys())
        if self.protocol_manager:
            names.update(self.protocol_manager.get_tool_names())
        return list(names)
