...
# --- Apps ---
app = typer.Typer(help="Kyberos: The Recursive Learning Agent")
dashboard_app = typer.Typer(help="Manage the Kyberos dashboard")
config_app = typer.Typer(help="Manage Kyberos configuration")
skills_app = typer.Typer(help="Manage Skills (Skills)")
pairing_app = typer.Typer(help="Manage Device/User Pairing")
memory_app = typer.Typer(help="Manage Agent Memory")
thread_app = typer.Typer(help="Manage Agent Thread")
...
console = Console()
PID_FILE = KYBEROS_ROOT / "kyberos.pid"
...
def send_message(text: str, wait: bool = True):
    ...
    if status != 200:
        console.print("[yellow]Daemon unreachable. Use 'kyberos start' to launch it.[/yellow]")
        return
...
def main(
    ...
    """Kyberos: The Recursive Learning Agent."""
    if message:
...
@app.command()
def start():
    """Start the Kyberos Daemon."""
    import os, psutil, atexit, asyncio
    from kyberos.core.daemon import run_daemon
...
def token_main(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        from kyberos.core.config import load_config
...
def token_new():
    ...
    import secrets
    from kyberos.core.config import load_config, ConfigLoader
    config = load_config()
...
def skills_list():
    ...
    for n, d in registry._skills.items():
...
def skills_create(name: str):
    ...
    path = Path("./.kyberos/archive").expanduser() / name
...
def skills_reload():
    ...
    status, _ = _api_request("/skills/reload", method="POST")
...
def config_set(key: str, value: str, is_json: bool = False):
    import json5
    from kyberos.core.config import KyberosConfig, ConfigLoader
    config = load_config().model_dump(mode='json')
    ...
    curr[keys[-1]] = val
    ConfigLoader.save(KyberosConfig(**config))
...
def pairing_list(protocol: str):
    from rich.table import Table
    from kyberos.core.pairing import PairingManager
    reqs = PairingManager().list_requests(protocol)
...
def pairing_approve(protocol: str, shortcode: str):
    from kyberos.core.pairing import PairingManager
    name = PairingManager().approve_request(protocol, shortcode)
...
def memory_reindex():
    ...
    async def run(): await ArchiveLibrarian().start_reindexing()
    asyncio.run(run())

@thread_app.command("reset")
def thread_reset(force: bool = False):
    from kyberos.memory.thread_manager import ThreadManager
    if not force and not typer.confirm("Reset thread?"): raise typer.Abort()
    ThreadManager(KYBEROS_ROOT / "memories" / "THREAD.md").clear()
    console.print("Thread reset.")

@thread_app.command("get")
def thread_get(raw: bool = False):
    f = KYBEROS_ROOT / "memories" / "THREAD.md"
