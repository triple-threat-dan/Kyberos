...
logger = logging.getLogger("kyberos.protocol.discord")

class KyberosDiscordClient(discord.Client):
    """Internal Discord Client to handle events."""
    
    def __init__(self, protocol: 'DiscordProtocol', *args, **kwargs):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if is_dm or self.user in message.mentions:
            should_respond = True
        elif re.match(rf"^{re.escape(self.protocol.agent_name)}\b\s*[,\:]?\s*", message.content.strip(), re.IGNORECASE):
            should_respond = True
        ...
        if message.author.bot:
             if await self._is_bot_loop(message.channel, limit=self.protocol.bot_loop_limit):
        ...
        if not pairing_mgr.is_user_allowed("discord", user_id, self.protocol.allowed_users):
            ...
            should_warn = is_dm or self.user in message.mentions or \
                          re.search(rf"\b{re.escape(self.protocol.agent_name)}\b", message.content, re.IGNORECASE)
        ...
        if self.protocol.allowed_channels and str(message.channel.id) not in self.protocol.allowed_channels:
             if not is_dm:
                 return

        if message.content.strip() == "/new":
             if not self.protocol.allowed_users or user_id not in self.protocol.allowed_users:
                 await message.channel.send("⛔ You are not authorized to reset the session.")
                 return
             await self.protocol.trigger_new_session(str(message.channel.id))
             return
...
class DiscordProtocol(BaseProtocol):
    def __init__(self, token: str, allowed_channels: List[str] = None, allowed_users: List[str] = None, 
        ...
        self.client: Optional[KyberosDiscordClient] = None
    ...
    async def start(self) -> None:
        ...
        logger.info("Initializing Discord Protocol...")
        ...
        self.client = KyberosDiscordClient(protocol=self, intents=intents)
        self._task = asyncio.create_task(self._run_client())
        logger.info("Discord Protocol background task started.")
    ...
    async def stop(self) -> None:
        ...
        logger.info("Stopping Discord Protocol...")
        ...
        if self._task:
            try:
                ...
                pass
        self.client = None
        logger.info("Discord Protocol stopped.")
    ...
    async def lookup_user(self, query: str) -> str:
        ...
        if not self.client or not self.client.is_ready():
            return "Error: Discord Protocol not ready."
    ...
    async def execute_tool(self, tool_name: str, args: Dict[str, Any]) -> Any:
        ...
        raise NotImplementedError(f"Tool {tool_name} not found in DiscordProtocol")
