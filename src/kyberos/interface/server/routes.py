...
logger = logging.getLogger("kyberos.routes")
...
async def get_status(request: Request):
    """Returns the current engine status, thread state, and recent history."""
    ...
    if thread_manager:
        try:
            thread_model = thread_manager.load()
            thread_data = thread_model.model_dump()
            thread_data['state'] = thread_model.state.value
        except Exception as e:
            thread_data = {"error": f"Thread load error: {e}"}
    else:
        thread_data = {"error": "ThreadManager not initialized"}
...
async def new_session(request: Request, body: NewSessionRequest = None):
    ...
    if context == "web":
        ...
        if thread_mgr := getattr(state, "thread_manager", None):
            thread_mgr.clear()
    else:
        if session_router:
...
async def trigger_heartbeat(request: Request):
    ...
    from kyberos.core.heartbeat import run_heartbeat_task
...
async def get_settings(request: Request):
    ...
    config: KyberosConfig = getattr(request.app.state, "config", None)
    if not config:
        raise HTTPException(status_code=500, detail="Configuration not loaded.")
    # Return dump using aliases to match the kyberos.json keys exactly
