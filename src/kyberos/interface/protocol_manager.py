...
class ProtocolManager:
    ...
    def __init__(
        ...
        self.adapters: Dict[str, BaseProtocol] = {}
        
        # Performance caches
        self._tool_map: Dict[str, BaseProtocol] = {}
    ...
    async def start(self) -> None:
        ...
        # Telegram
        if self.config.protocols.telegram.enabled and self.config.protocols.telegram.token:
            telegram = TelegramProtocol(token=self.config.protocols.telegram.token)
            telegram.on_message(self.handle_message)
            self.adapters["telegram"] = telegram
            await telegram.start()

        # Discord
        if self.config.protocols.discord.enabled and self.config.protocols.discord.token:
            discord = DiscordProtocol(
                token=self.config.protocols.discord.token,
                allowed_channels=self.config.protocols.discord.allowed_channels,
                allowed_users=self.config.protocols.discord.allowed_users,
                agent_name=self.config.agents.name,
                api_port=self.config.gateway.port,
                bot_loop_limit=self.config.protocols.discord.bot_loop_limit
            )
    ...
    async def handle_message(self, event: ProtocolEvent) -> None:
        """Central ingestion point for all platform messages."""
        logger.info(f"Incoming ProtocolEvent from {event.platform}: {event.content[:50]}")
    ...
    def get_all_tools_definitions(self) -> str:
        """Aggregates tool definitions from all enabled protocols."""
        definitions = [
    ...
    def get_tools_schema(self) -> List[Dict[str, Any]]:
        """Aggregates JSON schemas from all enabled protocols."""
        return self._cached_schemas if self._cached_schemas else []

    def get_tool_names(self) -> Set[str]:
        """Returns the set of all tool names available across all enabled protocols."""
        return self._cached_tool_names

    async def execute_tool(self, tool_name: str, args: Dict[str, Any]) -> Any:
        """Executes a tool by name across all active protocols."""
        adapter = self._tool_map.get(tool_name)
        if not adapter:
            raise ValueError(f"Tool {tool_name} not found in any active protocol.")
