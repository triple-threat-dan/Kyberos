...
logger = logging.getLogger("kyberos.librarian")
...
class ArchiveHandler(FileSystemEventHandler):
    """
    Handles file system events for the Archive.
...
class ArchiveLibrarian:
    """
    Service responsible for monitoring the Archive and managing the Vector Store.
    """

    def __init__(self, archive_path: Optional[Path] = None, memories_path: Optional[Path] = None):
        self.archive_path = archive_path or (KYBEROS_ROOT / "archive")
        self.memories_path = memories_path or (KYBEROS_ROOT / "memories")

        self.observer: Optional[Observer] = None
        self.event_handler: Optional[ArchiveHandler] = None
    ...
    def start(self) -> None:
        ...
        if not self.archive_path.exists():
            logger.warning(f"Archive directory {self.archive_path} does not exist. Creating it.")
        ...
        logger.info(f"Librarian starting watch on {self.archive_path} and {self.memories_path}")
        ...
        self.event_handler = ArchiveHandler(loop=loop, callback=self.index_file)

        self.observer = Observer()
        self.observer.schedule(self.event_handler, str(self.archive_path), recursive=True)
