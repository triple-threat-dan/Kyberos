"""
The Timeline & Dream Cycle.

This module contains the logic for the "Dream Cycle" — the agent's sleep phase
where it summarizes daily logs into long-term memory (Timeline).
"""
...
async def perform_dream_cycle(audit_logger, gateway, config) -> None:
    ...
    daily_log_path = KYBEROS_ROOT / "memories" / f"{today}.md"
...
"""
    try:
        ...
        if memory_updates:
            ...
            await _append_to_file(KYBEROS_ROOT / "memories" / "MEMORY.md", "".join(lines))
        ...
        if user_updates:
            ...
            await _append_to_file(KYBEROS_ROOT / "USER.md", "".join(lines))
        ...
        if heartbeat_updates:
            ...
            if await _append_to_file(KYBEROS_ROOT / "HEARTBEAT.md", "".join(lines)):
...
"""
                ...
                if dream_story:
                    dreams_path = KYBEROS_ROOT / "memories" / "DREAMS.md"
