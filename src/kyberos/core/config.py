"""
Configuration and Secrets Management for Kyberos.

This module handles loading configuration from disk (.kyberos/kyberos.json),
...
def find_kyberosroot() -> Path:
    """
    Locates the .kyberos directory. 
    Prioritizes the KYBEROS_ROOT environment variable, otherwise defaults 
    to the current directory's .kyberos folder.
    """
    if env_root := os.getenv("KYBEROS_ROOT"):
        return Path(env_root)
    return Path.cwd() / ".kyberos"

KYBEROS_ROOT = find_kyberosroot()
KYBEROS_CONFIG_FILE = "kyberos.json"
KYBEROS_WORKSPACE_DIR = KYBEROS_ROOT / "workspace"
KYBEROS_TEMPLATES_DIR = Path(__file__).parent.parent / "templates"
...
class KyberosConfig(BaseSettings):
    """Root configuration object for Kyberos."""
    debug: bool = False
...
class ConfigLoader:
    ...
    DEFAULT_CONFIG_DIR = KYBEROS_ROOT
    CONFIG_FILENAME = "kyberos.json"
    ...
    @classmethod
    def load(cls) -> KyberosConfig:
        """Loads .kyberos/kyberos.json. Creates default if missing."""
        config_path = cls.get_config_path()
        cls._ensure_permissions(config_path)

        if not config_path.exists():
            logger.info(f"No config found at {config_path}. Creating default.")
            default_config = KyberosConfig()
        ...
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                data = json5.load(f)
            return KyberosConfig(**data)
        except Exception as e:
            logger.error(f"Failed to load configuration from {config_path}: {e}")
            raise ValueError(f"Invalid configuration file: {e}")

    @classmethod
    def save(cls, config: KyberosConfig) -> None:
...
_params: Optional[KyberosConfig] = None
_secrets: Optional[SecretsManager] = None

def load_config() -> KyberosConfig:
