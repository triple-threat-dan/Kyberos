"""
The KyberosDaemon: Core orchestration engine for Kyberos.
...
from kyberos.core.bootstrap import ensure_workspace
from kyberos.core.config import KYBEROS_ROOT, ConfigLoader, load_config
from kyberos.core.database import AuditLogger
from kyberos.core.session_router import SessionRouter
from kyberos.interface.server.routes import router as dashboard_router

logger = logging.getLogger("kyberos.daemon")
console = Console()
...
async def shutdown_daemon(
    ...
    # 2. Stop Protocol Manager (Discord/Telegram)
    try:
        await protocol_manager.stop()
    except Exception as e:
        logger.error(f"ProtocolManager stop failed: {e}")
...
async def run_daemon(tui_app: Optional[App], api_app: FastAPI) -> None:
    """
    Entry point for the Kyberos Daemon.
    ...
    """
    config = load_config()
    logger.info(f"Kyberos Root: {KYBEROS_ROOT}")
    ...
    if not config.gateway.web_ui_token:
        ...
        console.print("Use 'kyberos token' to retrieve it later.")

    logger.info(f"Starting Kyberos Daemon (PID {os.getpid()})...")
    ...
    from kyberos.core.heartbeat import HeartbeatManager, run_heartbeat_task
    ...
    session_router = SessionRouter(KYBEROS_ROOT / "active_sessions.json")

    from kyberos.interface.protocol_manager import ProtocolManager
    protocol_manager = ProtocolManager(config, audit_logger, command_bus, internal_bus, session_router)
    await protocol_manager.start()
    logger.info("ProtocolManager started.")
    ...
    librarian = ArchiveLibrarian()
    librarian.start()
    
    thread_path = KYBEROS_ROOT / "memories" / "THREAD.md"
    thread_manager = ThreadManager(thread_path)
    ...
    api_app.state.thread_manager = thread_manager
    ...
    if not last_session_id:
         logger.info("Fresh session detected. Clearing Thread.")
    ...
    from kyberos.brain.rlm import RLMEngine
    ...
    async def brain_loop():
        ...
        async def process_item(item):
            ...
            # Session Management
            if source == "PROTOCOL" and not session_id:
                ...
                if not await audit_logger.get_session(session_id):
                    name = f"Protocol Session {session_id[:8]}"
            ...
            try:
                if source == "PROTOCOL" and platform and sender_id:
                    asyncio.create_task(protocol_manager.trigger_typing(platform, sender_id))
                ...
                if response:
                    if source in ["WEB", "HEARTBEAT", "CLI"]:
                        ...
                        adapter = protocol_manager.adapters.get(platform)
                        if adapter:
                ...
                if source == "WEB":
                    ...
                    try:
                        ...
                        pass
                    await internal_bus.put({"level": "ERROR", "message": f"PROTOCOL ({platform}) error: {e}", "source": "BRAIN"})
            finally:
                if source == "PROTOCOL" and platform and sender_id:
                    await protocol_manager.stop_typing(platform, sender_id)
    ...
    try:
        logger.info("KyberosDaemon Running (Ctrl+C to stop)...")
