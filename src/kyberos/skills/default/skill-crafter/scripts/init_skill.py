...
if __name__ == "__main__":
    ...
    try:
        ...
        if input_arg.strip().startswith("{"):
            ...
            name = args.get("name")
            # Optional path override, default to Archive
        ...
        # Default to ./.kyberos/archive (CWD) if no path provided
        if custom_path:
             target_dir = Path(custom_path).resolve()
        else:
             target_dir = Path.cwd() / ".kyberos" / "archive"

        if not target_dir.parent.exists():
            print(f"Warning: .kyberos directory not found at {target_dir.parent}")
