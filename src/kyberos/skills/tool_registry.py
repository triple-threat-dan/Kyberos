"""
Tool Registry and Gateway Manager for Kyberos.

Central hub for tool discovery, execution, and schema generation.
Manages internal standard library tools and external skills.
"""
...
from kyberos.skills.sandbox import SandboxManager

logger = logging.getLogger("kyberos.skills")
...
class ToolRegistry:
    ...
    def __init__(self, config: KyberosConfig, librarian=None, audit_logger=None, session_router=None):
        self.config = config
        ...
        self._cached_skills_context: Optional[str] = None
        ...
        self.skills_dir = KYBEROS_ROOT / "archive"
        self.load_skills()
    ...
    def load_skills(self):
        """Scans the archive and loads valid SKILL.md files."""
        self._skills = {}
        self._cached_skills_context = None
    ...
    def get_internal_tools_context(self) -> str:
        ...
        lines.append("- **CRITICAL**: Use memory_search to query your Archive/Memories for context. PREFER this over direct file reads.")
    ...
    def get_skills_context(self) -> str:
        """Returns summarized skills documentation."""
        if self._cached_skills_context:
            return self._cached_skills_context

        if not self._skills:
            return ""

        lines = ["## Available Skills", "Use `spell_crafter` to initiate a spell.", ""]
        cwd = Path.cwd()
    ...
    def memory_search(self, query: str) -> str:
        """Search the agent's Archive for relevant memories."""
