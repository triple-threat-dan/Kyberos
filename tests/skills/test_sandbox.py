...
from kyberos.skills.sandbox import SandboxManager, SecurityViolationError


@pytest.fixture
def mock_config():
    config = MagicMock(spec=KyberosConfig)
...
def temp_home(tmp_path):
    with patch("kyberos.skills.sandbox.Path.home", return_value=tmp_path):
        yield tmp_path
...
def test_init_paths(manager, temp_home):
    assert manager.sandbox_dir == temp_home / ".kyberos" / ".kyberos_sandbox"
    assert manager.temp_dir == temp_home / ".kyberos" / "temp"
...
def test_get_uv_path_in_path(manager):
    with patch("kyberos.skills.sandbox.shutil.which", return_value="/usr/bin/uv"):
        assert manager._get_uv_path() == "/usr/bin/uv"


def test_get_uv_path_in_package(manager):
    with patch("kyberos.skills.sandbox.shutil.which", return_value=None):
...
async def test_ensure_environment_new_success(manager, temp_home):
    with patch.object(manager, "_get_uv_path", return_value="/usr/bin/uv"):
        ...
        with patch("kyberos.skills.sandbox.asyncio.create_subprocess_exec", return_value=mock_proc) as mock_exec:
...
async def test_ensure_environment_venv_fails(manager, temp_home):
    with patch.object(manager, "_get_uv_path", return_value="/usr/bin/uv"):
        ...
        with patch("kyberos.skills.sandbox.asyncio.create_subprocess_exec", return_value=mock_proc):
...
async def test_ensure_environment_pip_fails(manager, temp_home):
    with patch.object(manager, "_get_uv_path", return_value="/usr/bin/uv"):
        ...
        with patch("kyberos.skills.sandbox.asyncio.create_subprocess_exec", side_effect=[mock_proc_venv, mock_proc_pip]):
...
async def test_ensure_environment_existing_broken(manager, temp_home):
    ...
    with patch.object(manager, "_get_uv_path", return_value="/usr/bin/uv"):
        ...
        with patch("kyberos.skills.sandbox.asyncio.create_subprocess_exec", side_effect=[mock_proc_check, mock_proc_venv, mock_proc_pip]):
            await manager.ensure_environment()
...
async def test_ensure_environment_existing_check_exception(manager, temp_home):
    ...
    with patch.object(manager, "_get_uv_path", return_value="/usr/bin/uv"):
        ...
        with patch("kyberos.skills.sandbox.asyncio.create_subprocess_exec", side_effect=[Exception("Check failed"), mock_proc_venv, mock_proc_pip]):
            await manager.ensure_environment()
...
async def test_run_python_timeout(manager, temp_home):
    ...
    with patch("kyberos.skills.sandbox.asyncio.create_subprocess_exec", return_value=mock_proc):
        with patch("kyberos.skills.sandbox.asyncio.wait_for", side_effect=mock_wait_for):
...
async def test_run_python_cleanup_fails(manager, temp_home):
    ...
    with patch("kyberos.skills.sandbox.asyncio.create_subprocess_exec", return_value=mock_proc):
        with patch("pathlib.Path.unlink", side_effect=Exception("unlink failed")):
            with patch("kyberos.skills.sandbox.logger.warning") as mock_logger:
...
def test_init_paths_non_win32(manager, temp_home):
    with patch("kyberos.skills.sandbox.sys.platform", "linux"):
