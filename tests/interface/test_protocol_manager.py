...
@pytest.mark.asyncio
@patch("kyberos.interface.protocol_manager.TelegramProtocol")
async def test_start_telegram_enabled(mock_telegram_class, mock_config, mock_audit):
    mock_config.protocols.telegram.enabled = True
    mock_config.protocols.telegram.token = "test_telegram_token"
...
@pytest.mark.asyncio
@patch("kyberos.interface.protocol_manager.DiscordProtocol")
async def test_start_discord_enabled(mock_discord_class, mock_config, mock_audit):
    mock_config.protocols.discord.enabled = True
    mock_config.protocols.discord.token = "test_discord_token"
    mock_config.protocols.discord.allowed_channels = ["123"]
    mock_config.protocols.discord.allowed_users = ["456"]
    mock_config.agents.name = "TestAgent"
    mock_config.gateway.port = 8000
    mock_config.protocols.discord.bot_loop_limit = 5
...
@pytest.mark.asyncio
async def test_start_no_protocols(manager, caplog):
    import logging
    with caplog.at_level(logging.WARNING):
        await manager.start()
        
    assert not manager.adapters
    assert "No Protocols enabled. Agent is lonely." in caplog.text
...
async def test_handle_message_no_pending_task(manager):
    ...
    event = ProtocolEvent(platform="telegram", sender_id="user1", content="hello")
...
async def test_handle_message_approval_with_adapter(manager):
    ...
    event = ProtocolEvent(platform="discord", sender_id="user1", content="yes, go ahead")
    await manager.handle_message(event)
...
async def test_handle_message_approval_no_adapter(manager):
    ...
    event = ProtocolEvent(platform="telegram", sender_id="user1", content="approve")
    await manager.handle_message(event)
...
async def test_handle_message_denial_with_adapter(manager):
    ...
    event = ProtocolEvent(platform="telegram", sender_id="user1", content="stop that right now")
    await manager.handle_message(event)
...
async def test_handle_message_denial_no_adapter(manager):
    ...
    event = ProtocolEvent(platform="discord", sender_id="user1", content="no")
    await manager.handle_message(event)
...
async def test_handle_message_pending_task_not_approval_or_denial(manager):
    ...
    event = ProtocolEvent(platform="discord", sender_id="user1", content="what is the task about?")
    await manager.handle_message(event)
...
async def test_execute_tool_no_adapters(manager):
    with pytest.raises(ValueError, match="Tool some_tool not found in any active protocol."):
