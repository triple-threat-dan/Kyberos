...
@pytest.fixture
def telegram_protocol():
    return TelegramProtocol(token="fake_token")
...
@pytest.mark.asyncio
async def test_telegram_protocol_start(telegram_protocol):
    ...
    with patch("telegram.ext.Application.builder", return_value=mock_builder):
        await telegram_protocol.start()
        
        assert telegram_protocol._started is True
...
@pytest.mark.asyncio
async def test_telegram_protocol_start_already_started(telegram_protocol):
    telegram_protocol._started = True
    with patch("telegram.ext.Application.builder") as mock_builder:
        await telegram_protocol.start()
        mock_builder.assert_not_called()

@pytest.mark.asyncio
async def test_telegram_protocol_stop(telegram_protocol):
    mock_app = MagicMock(spec=Application)
    ...
    telegram_protocol.application = mock_app
    telegram_protocol._started = True
    
    await telegram_protocol.stop()
    
    assert telegram_protocol._started is False
    mock_app.updater.stop.assert_called_once()
    mock_app.stop.assert_called_once()
    mock_app.shutdown.assert_called_once()

@pytest.mark.asyncio
async def test_telegram_protocol_stop_not_started(telegram_protocol):
    await telegram_protocol.stop()
    # Should not raise error or call anything

@pytest.mark.asyncio
async def test_telegram_protocol_send_message_success(telegram_protocol):
    mock_app = MagicMock(spec=Application)
    mock_app.bot.send_message = AsyncMock()
    telegram_protocol.application = mock_app
    telegram_protocol._started = True
    
    await telegram_protocol.send_message("12345", "Test message")
    
    mock_app.bot.send_message.assert_called_once_with(chat_id="12345", text="Test message")

@pytest.mark.asyncio
async def test_telegram_protocol_send_message_not_started(telegram_protocol, caplog):
    await telegram_protocol.send_message("12345", "Test message")
    assert "Cannot send message: Telegram Protocol not started." in caplog.text

@pytest.mark.asyncio
async def test_telegram_protocol_trigger_typing(telegram_protocol):
    mock_app = MagicMock()
    ...
    with patch("kyberos.interface.adapters.telegram.asyncio.sleep", side_effect=mock_sleep_yield) as mock_sleep:
        await telegram_protocol.trigger_typing("12345")
        ...
        assert "12345" in telegram_protocol._typing_tasks
        mock_app.bot.send_chat_action.assert_called_with(chat_id="12345", action=ChatAction.TYPING)
        
        await telegram_protocol.stop_typing("12345")
        assert "12345" not in telegram_protocol._typing_tasks

@pytest.mark.asyncio
async def test_telegram_handle_message(telegram_protocol, mock_update, mock_context):
    received_event = None
    async def handler(event):
        nonlocal received_event
        received_event = event
    
    telegram_protocol.on_message(handler)
    
    await telegram_protocol._telegram_handle_message(mock_update, mock_context)
    
    assert received_event is not None
    assert received_event.platform == "telegram"
    assert received_event.sender_id == "12345"
    assert received_event.content == "Hello Kyberos"
    assert received_event.metadata["username"] == "testuser"
    assert received_event.metadata["message_id"] == 67890

@pytest.mark.asyncio
async def test_telegram_handle_message_reply(telegram_protocol, mock_update, mock_context):
    received_event = None
    async def handler(event):
        nonlocal received_event
        received_event = event
...
@pytest.mark.asyncio
async def test_telegram_handle_message_no_text(telegram_protocol, mock_update, mock_context):
    received_event = None
    async def handler(event):
        nonlocal received_event
        received_event = event
