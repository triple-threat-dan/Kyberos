...
from kyberos.interface.adapters.discord import DiscordProtocol, KyberosDiscordClient
from kyberos.interface.adapters.base import ProtocolEvent


@pytest.fixture
def mock_discord_protocol():
...
@pytest.fixture
def mock_client(mock_discord_protocol):
    intents = discord.Intents.default()
    client = KyberosDiscordClient(protocol=mock_discord_protocol, intents=intents)
    mock_discord_protocol.client = client
    # Set to ready state
...
# --- KyberosDiscordClient Tests ---

import logging

@pytest.mark.asyncio
async def test_kyberosclient_on_ready(mock_client, caplog):
...
@pytest.mark.asyncio
async def test_kyberosclient_is_bot_loop(mock_client):
...
@pytest.mark.asyncio
@patch("kyberos.core.pairing.PairingManager")
async def test_on_message_triggers(mock_pairing_cls, mock_client):
    ...
    mock_client.protocol.allowed_users = ["human_id"]
    mock_msg.author.id = "human_id"
    mock_msg.channel.id = "111"  # Needs to pass channel whitelist
    
    # We'll mock the internal protocol _emit to verify if should_respond was True
    mock_client.protocol._emit = AsyncMock()
    ...
    await mock_client.on_message(mock_msg)
    mock_client.protocol._emit.assert_not_called()

    # 2. DM Channel -> Should Respond
    mock_msg.channel = MagicMock(spec=discord.DMChannel) # Setting specific spec helps isinstance
    await mock_client.on_message(mock_msg)
    mock_client.protocol._emit.assert_called_once()
    mock_client.protocol._emit.reset_mock()
    ...
    mock_msg.content = "Kyberos, do something"
    await mock_client.on_message(mock_msg)
    mock_client.protocol._emit.assert_called_once()
    mock_client.protocol._emit.reset_mock()
    mock_msg.content = "random chat"
...
async def test_on_message_auth_rejection(mock_pairing_cls, mock_client):
    ...
    mock_msg.mentions = []
    mock_msg.content = "Kyberos, tell me a joke"
    mock_msg.channel.send.reset_mock()
    await mock_client.on_message(mock_msg)
    mock_msg.channel.send.assert_called_once()
...
async def test_on_message_slash_new(mock_pairing_cls, mock_client):
    ...
    mock_client.protocol.trigger_new_session = AsyncMock()
    
    await mock_client.on_message(mock_msg)
    mock_client.protocol.trigger_new_session.assert_called_with("111")
...
async def test_on_message_formatting_and_emit(mock_pairing_cls, mock_client):
    ...
    mock_client.protocol._emit.assert_called_once()
    event: ProtocolEvent = mock_client.protocol._emit.call_args[0][0]
...
# --- DiscordProtocol Tests ---

@pytest.mark.asyncio
@patch("aiohttp.ClientSession.post")
async def test_trigger_new_session(mock_post, mock_discord_protocol):
    mock_discord_protocol.send_message = AsyncMock()
    ...
    await mock_discord_protocol.trigger_new_session("target_1")
    
    mock_discord_protocol.send_message.assert_called_with("target_1", "🔄 **Session Reset**. New ID: `sid123`")
    assert mock_post.call_args[1]["json"] == {"context": "discord:target_1"}
    
    # 2. Failure 500
    mock_ctx.status = 500
    await mock_discord_protocol.trigger_new_session("target_1")
    mock_discord_protocol.send_message.assert_called_with("target_1", "⚠️ Failed to reset session. API Status: 500")
    
    # 3. Exception
    mock_post.side_effect = Exception("Network Down")
    await mock_discord_protocol.trigger_new_session("target_1")
    mock_discord_protocol.send_message.assert_called_with("target_1", "⚠️ Error triggering new session: Network Down")


@pytest.mark.asyncio
@patch("kyberos.interface.adapters.discord.KyberosDiscordClient")
async def test_start_and_stop(mock_client_cls, mock_discord_protocol):
    mock_instance = AsyncMock()
    mock_client_cls.return_value = mock_instance
    
    # Test Start
    await mock_discord_protocol.start()
    assert mock_discord_protocol.client is not None
    assert mock_discord_protocol._task is not None
    ...
    # Test Stop
    await mock_discord_protocol.stop()
    mock_instance.close.assert_called_once()
    assert mock_discord_protocol.client is None


def test_chunk_message():
    content = "A" * 2500
    chunks = DiscordProtocol._chunk_message(content, max_length=2000)
    assert len(chunks) == 2
...
@pytest.mark.asyncio
async def test_send_dm(mock_discord_protocol, mock_client):
    mock_discord_protocol.stop_typing = AsyncMock()
    ...
    res = await mock_discord_protocol.send_dm("123", "Hello")
    assert "Message sent" in res
    mock_user.send.assert_called_once_with("Hello")
    mock_discord_protocol.stop_typing.assert_called_with("123")


@pytest.mark.asyncio
async def test_send_dm_fallback_name(mock_discord_protocol, mock_client):
    mock_discord_protocol.stop_typing = AsyncMock()
    ...
    res = await mock_discord_protocol.send_dm("John", "Hi")
    assert "Message sent" in res
    mock_user.send.assert_called_once_with("Hi")


@pytest.mark.asyncio
async def test_send_channel_message(mock_discord_protocol, mock_client):
    mock_discord_protocol.stop_typing = AsyncMock()
    
    mock_channel = AsyncMock()
    mock_client.get_channel.return_value = mock_channel
    
    await mock_discord_protocol.send_channel_message("111", "Hello Channel")
    mock_channel.send.assert_called_once_with("Hello Channel")


@pytest.mark.asyncio
async def test_send_message_router(mock_discord_protocol):
    mock_discord_protocol.send_channel_message = AsyncMock()
    mock_discord_protocol.send_dm = AsyncMock()
    
    # Success on channel
    await mock_discord_protocol.send_message("111", "test")
    mock_discord_protocol.send_channel_message.assert_called_once()
    mock_discord_protocol.send_dm.assert_not_called()
    
    # Fail on channel, fallback to DM
    mock_discord_protocol.send_channel_message.side_effect = Exception("Not a channel")
    await mock_discord_protocol.send_message("111", "test")
    mock_discord_protocol.send_dm.assert_called_once()


@pytest.mark.asyncio
async def test_add_reaction(mock_discord_protocol, mock_client):
    mock_channel = AsyncMock()
    mock_msg = AsyncMock()
    mock_client.get_channel.return_value = mock_channel
    mock_channel.fetch_message.return_value = mock_msg
    
    await mock_discord_protocol.add_reaction("111", "222", "👍")
    mock_msg.add_reaction.assert_called_once_with("👍")


@pytest.mark.asyncio
async def test_lookup_user(mock_discord_protocol, mock_client):
    mock_user = MagicMock()
    mock_user.name = "Bob"
    mock_user.id = 444
    ...
    res = await mock_discord_protocol.lookup_user("bob")
    assert "Bob" in res
    assert "444" in res


@pytest.mark.asyncio
async def test_typing_indicators(mock_discord_protocol, mock_client):
    ...
    # Trigger
    await mock_discord_protocol.trigger_typing("111")
    assert "111" in mock_discord_protocol._typing_tasks
    
    # Allow task to spin up
    await asyncio.sleep(0.01)
    
    # Stop
    await mock_discord_protocol.stop_typing("111")
    assert "111" not in mock_discord_protocol._typing_tasks
    # Ensure it's done/cancelled
    await asyncio.sleep(0.01)


@pytest.mark.asyncio
async def test_tools_interface(mock_discord_protocol, mock_client):
    mock_discord_protocol.send_dm = AsyncMock(return_value="OK")
    mock_discord_protocol.lookup_user = AsyncMock()
    mock_discord_protocol.send_channel_message = AsyncMock()
    mock_discord_protocol.add_reaction = AsyncMock()
    
    assert isinstance(mock_discord_protocol.get_tools_definition(), str)
    assert len(mock_discord_protocol.get_tool_names()) == 4
    assert len(mock_discord_protocol.get_tools_schema()) == 4
    
    # Execute Tool Calls
    res = await mock_discord_protocol.execute_tool("discord_send_dm", {"user_id": "1", "content": "A"})
    assert res == "OK"
    mock_discord_protocol.send_dm.assert_called_once()
    
    await mock_discord_protocol.execute_tool("discord_lookup_user", {"query": "A"})
    mock_discord_protocol.lookup_user.assert_called_once()
    
    await mock_discord_protocol.execute_tool("discord_send_channel_message", {"channel_id": "1", "content": "A"})
    mock_discord_protocol.send_channel_message.assert_called_once()
    
    await mock_discord_protocol.execute_tool("discord_add_reaction", {"channel_id": "1", "message_id": "2", "emoji": "A"})
    mock_discord_protocol.add_reaction.assert_called_once()
    
    # Missing tool
    with pytest.raises(NotImplementedError):
        await mock_discord_protocol.execute_tool("invalid_tool", {})

# --- Coverage Edge Cases ---

@pytest.mark.asyncio
async def test_run_client_exceptions(mock_discord_protocol, mock_client):
    # Test CancelledError
    async def mock_start(*args):
        raise asyncio.CancelledError()
    mock_client.start = mock_start
    await mock_discord_protocol._run_client()  # Should handle quietly

    # Test Generic Exception
    async def mock_start_exc(*args):
        raise Exception("Fatal Crash")
    mock_client.start = mock_start_exc
    await mock_discord_protocol._run_client()  # Should log quietly
    
@pytest.mark.asyncio
async def test_stop_no_client(mock_discord_protocol):
    mock_discord_protocol.client = None
    await mock_discord_protocol.stop()  # Should return early

@pytest.mark.asyncio
async def test_stop_cancelled_task(mock_discord_protocol, mock_client):
    mock_client.close = AsyncMock()
    mock_discord_protocol._task = asyncio.create_task(asyncio.sleep(10))
    mock_discord_protocol._task.cancel()
    await mock_discord_protocol.stop() # Should suppress CancelledError

@pytest.mark.asyncio
async def test_send_dm_not_ready(mock_discord_protocol, mock_client):
    mock_discord_protocol.client.is_ready.return_value = False
    assert await mock_discord_protocol.send_dm("1", "msg") is None

@pytest.mark.asyncio
async def test_send_channel_message_not_ready(mock_discord_protocol, mock_client):
    mock_discord_protocol.client.is_ready.return_value = False
    assert await mock_discord_protocol.send_channel_message("1", "msg") is None

@pytest.mark.asyncio
async def test_lookup_user_not_ready(mock_discord_protocol, mock_client):
    mock_discord_protocol.client.is_ready.return_value = False
    assert await mock_discord_protocol.lookup_user("A") == "Error: Discord Protocol not ready."

@pytest.mark.asyncio
async def test_add_reaction_not_ready(mock_discord_protocol, mock_client):
    mock_discord_protocol.client.is_ready.return_value = False
    assert await mock_discord_protocol.add_reaction("1", "1", "a") is None

@pytest.mark.asyncio
async def test_typing_indicators_not_ready(mock_discord_protocol, mock_client):
    mock_discord_protocol.client.is_ready.return_value = False
    await mock_discord_protocol.trigger_typing("1")
    assert "1" not in mock_discord_protocol._typing_tasks

@pytest.mark.asyncio
async def test_send_dm_global_name_match(mock_discord_protocol, mock_client):
    mock_discord_protocol.stop_typing = AsyncMock()
    ...
    res = await mock_discord_protocol.send_dm("Bob", "Hi")
    assert "Message sent" in res

@pytest.mark.asyncio
async def test_send_dm_display_name_match(mock_discord_protocol, mock_client):
    mock_discord_protocol.stop_typing = AsyncMock()
    mock_client.fetch_user.side_effect = Exception("Not an ID")
    type(mock_client).users = PropertyMock(return_value=[])
    ...
    mock_user.send = AsyncMock()
    res = await mock_discord_protocol.send_dm("charlie", "Hi")
    assert "Message sent" in res

@pytest.mark.asyncio
async def test_send_dm_not_found_debug_msg(mock_discord_protocol, mock_client):
    mock_discord_protocol.stop_typing = AsyncMock()
    ...
    res = await mock_discord_protocol.send_dm("Nobody", "Hi")
    assert "not found" in res
    assert "TestG" in res

@pytest.mark.asyncio
async def test_send_dm_generic_exception(mock_discord_protocol, mock_client):
    mock_client.fetch_user.side_effect = Exception("Fatal")
    type(mock_client).users = PropertyMock(side_effect=Exception("Iter crash"))
    res = await mock_discord_protocol.send_dm("123", "msg")
    assert "Error sending DM" in res

@pytest.mark.asyncio
async def test_lookup_user_id_fetch_none(mock_discord_protocol, mock_client):
    # 1. Return None
    mock_client.fetch_user.return_value = None
    res = await mock_discord_protocol.lookup_user("12345")
    assert "No users found" in res
    
    # 2. Raise exception (coverage 391-393)
    mock_client.fetch_user.side_effect = Exception()
    res2 = await mock_discord_protocol.lookup_user("54321")
    assert "No users found" in res2

@pytest.mark.asyncio
async def test_lookup_user_id_fetch_success(mock_discord_protocol, mock_client):
    ...
    res = await mock_discord_protocol.lookup_user("54321")
    assert "Direct Fetch" in res
    assert "Bob" in res

@pytest.mark.asyncio
async def test_send_channel_message_fetch_fallback_and_no_send(mock_discord_protocol, mock_client):
    mock_discord_protocol.stop_typing = AsyncMock()
    mock_client.get_channel.return_value = None
    
    # 1. Fetch channel fails (hits line 466-467)
    mock_client.fetch_channel.side_effect = Exception("Not found")
    await mock_discord_protocol.send_channel_message("111", "msg")
...
@pytest.mark.asyncio
async def test_start_running_client(mock_discord_protocol):
    mock_discord_protocol.client = MagicMock()
    await mock_discord_protocol.start() # Hits early return

@pytest.mark.asyncio
async def test_lookup_user_skip_self_and_dupes(mock_discord_protocol, mock_client):
...
@pytest.mark.asyncio
async def test_send_dm_skip_self_cache(mock_discord_protocol, mock_client):
    mock_discord_protocol.stop_typing = AsyncMock()
    mock_client.fetch_user.side_effect = Exception()
    ...
    with patch("discord.utils.get", side_effect=[mock_client.user, mock_num_user, None, None, None, None]):
        ...
        await mock_discord_protocol.send_dm("123", "msg")
    ...
    with patch("discord.utils.get", return_value=None):
        await mock_discord_protocol.send_dm("john", "msg")
        mock_ci_user.send.assert_called_once()

@pytest.mark.asyncio
async def test_stop_typing_cancelled(mock_discord_protocol):
    async def cancel_me(): raise asyncio.CancelledError()
    mock_discord_protocol._typing_tasks["111"] = asyncio.create_task(cancel_me())
    await mock_discord_protocol.stop_typing("111")
    assert "111" not in mock_discord_protocol._typing_tasks
    
@pytest.mark.asyncio
async def test_get_tools_definition_no_file(mock_discord_protocol):
    with patch("kyberos.interface.adapters.discord.Path.exists", return_value=False):
        assert mock_discord_protocol.get_tools_definition() == ""

@pytest.mark.asyncio
async def test_send_channel_message_generic_exception(mock_discord_protocol, mock_client):
    mock_client.get_channel.side_effect = Exception("Fail")
    await mock_discord_protocol.send_channel_message("111", "msg")

@pytest.mark.asyncio
async def test_add_reaction_exceptions_and_fallbacks(mock_discord_protocol, mock_client):
    # 1. Channel not found
    mock_client.get_channel.return_value = None
    mock_client.fetch_channel.side_effect = Exception()
    await mock_discord_protocol.add_reaction("1", "1", "A") # Error Channel not found
    
    # 2. Generic exception
    mock_client.get_channel.side_effect = Exception("Crash")
    await mock_discord_protocol.add_reaction("1", "1", "A")

@pytest.mark.asyncio
async def test_typing_exceptions_and_fallbacks(mock_discord_protocol, mock_client):
    # Already typing
    task = asyncio.create_task(asyncio.sleep(2))
    mock_discord_protocol._typing_tasks["111"] = task
    await mock_discord_protocol.trigger_typing("111") # Should exit early
    task.cancel()
    ...
    await mock_discord_protocol.trigger_typing("222")
    ...
    mock_client.fetch_channel.side_effect = Exception()
    mock_client.fetch_user.side_effect = Exception()
    await mock_discord_protocol.trigger_typing("444")
    await asyncio.sleep(0.01)
    
    # Loop Crash
    mock_client.get_channel.side_effect = Exception("Boom")
    await mock_discord_protocol.trigger_typing("333")
    await asyncio.sleep(0.01)
